        ...
        <div className="space-y-16 sm:space-y-32 relative">
          {timelineItems.map((item) => (
            <div key={item.id} className="flex items-center relative">
              {/* Outer Circle */}
              <div className={`absolute left-1/2 transform -translate-x-1/2 w-8 sm:w-14 h-8 sm:h-14 rounded-full 
                ${item.completed ? 'bg-[#333333]' : 'bg-[#333333]'}`}>
                {/* Inner Circle */}
                <div className={`absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 
                  w-3 sm:w-5 h-3 sm:h-5 rounded-full ${item.completed ? 'bg-[#6be246]' : 'bg-white'}`} />
              </div>
              ...
              <div 
                className={`absolute ${
                  item.align === 'left'
                    ...
                    : item.align === 'right' && 'text-center sm:text-left left-[70%] transform -translate-x-1/2 sm:translate-x-0 sm:left-[calc(50%+3rem)]'
                } w-[45%] sm:w-auto`}
              >
